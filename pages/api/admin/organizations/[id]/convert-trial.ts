...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (sub.status === 'ACTIVE') {
    ...
    try {
      ...
      await db.activitylog.create({ data: { organizationId: orgId, userId: userId || null, action: 'TRIAL_CONVERT', details: { previousStatus: sub.status, idempotent: true } } })
    } catch {}
  ...
  try {
    const userId = (token as any)?.id as string | undefined
    await db.activitylog.create({ data: { organizationId: orgId, userId: userId || null, action: 'TRIAL_CONVERT', details: { previousStatus: sub.status } } })
  } catch {}
